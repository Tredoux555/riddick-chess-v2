...
router.get('/:analysisId', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ analysisId: analysis.id, status: 'completed', whiteAccuracy: analysis.white_accuracy, blackAccuracy: analysis.black_accuracy, summary: analysis.analysis_data?.summary, moves: movesResult.rows, completedAt: analysis.completed_at });
  } catch (err) {
